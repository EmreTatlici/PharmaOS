...
public async Task<ActionResult<IEnumerable<SaleHistoryResponse>>> GetSales()
{
    var sales = await _context.StockMovements
        ...
        .Select(x => new SaleHistoryResponse
        {
            ...
            BatchNumber = x.InventoryItem != null ? x.InventoryItem.BatchNumber : "",
            Quantity = x.Quantity,
...
var startOfDayUtc = TimeZoneInfo.ConvertTimeToUtc(
    DateTime.SpecifyKind(startOfDayTurkey, DateTimeKind.Unspecified),
    turkeyTimeZone);

var endOfDayUtc = TimeZoneInfo.ConvertTimeToUtc(
    DateTime.SpecifyKind(endOfDayTurkey, DateTimeKind.Unspecified),
    turkeyTimeZone);
...
var sales = await _context.StockMovements
    ...
    var response = new DailySalesResponse
    {
        ...
        TotalAmount = sales.Sum(x => x.Quantity * (x.UnitSalePrice ?? 0)),
        CashAmount = sales
            .Where(x => x.PaymentType == "Cash")
            .Sum(x => x.Quantity * (x.UnitSalePrice ?? 0)),
        CardAmount = sales
...
[HttpPost]
public async Task<ActionResult> CreateSale(CreateSaleRequest request)
{
    if (string.IsNullOrWhiteSpace(request.PaymentType))
{
    ...
    {
        return BadRequest("Satış miktarı sıfırdan büyük olmalıdır.");
    }
    ...
    return Ok(new
    {
        ...
        fefoWarning =
            ...
            ? new
            {
                message = "Rafta daha eski SKT'li lot bulunuyor.",
                recommendedBatch = recommendedLot.BatchNumber,
                recommendedExpirationDate = recommendedLot.ExpirationDate,
                availableQuantity = recommendedLot.Quantity
            }
            : null
    });
}
